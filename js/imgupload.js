+function($){
    ...
    Imgupload.prototype={
        ...
        initCanvas:function(){
            ...
            //添加剪裁框 并绘制默认剪裁框
            self._$canvasCrop =$('<canvas '="" +="" 'width="' + self._imgCropSize  + '" height="' + self._imgCropSize + '">' +
                '</canvas>');
            ...
            // 绘制预览canvas
            self._$canvasPreview =$('<canvas '="" +="" 'width="' + self._imgPreSize  + '" height="' + self._imgPreSize + '">' +
                '</canvas>');
        ...
        buildBox:function(src){
            ...
            function drawImg() {
                ...
                if(self._img.width<self._imgpresize){ alert("图片宽度不能小于"+self._imgpresize+"px");="" return="" false;="" }="" if(self._img.height<self._imgpresize){="" alert("图片高度不能小于"+self._imgpresize+"px");="" 设置drawimage="" 合适的="" 高宽使图片刚好不变形的显示在canvas内="" 检查图片尺寸="" 并调整以适应="" if(self._imgscale="">1){
        ...
        drawCanvasCrop:function(){
            ...
            if(self._img_sx<0){ self._imgcrop_sx="-((self._$canvasW" -="" self._imgcropsize)="" 2-self._img_sx)="" }="" if(self._img_sy<0){="" self._imgcrop_sy="-((self._$canvasH" 2-self._img_sy);="" self._$canvascrop.ctx.drawimage(self._img,="" self._imgcrop_sx,="" self._imgcrop_sy,="" self._img.width,="" self._img.height);="" },="" 移动="" 或="" 者放大缩小="" 检测图片是否超出剪裁框="" checkcanvasxy:function(){="" var="" self="this;" 检测_img_sx="" 不让图片移除剪裁框范围以内="" if((-self._img_sx+(self._$canvasw="" 2+self._imgcropsize)="">=self._imgW){
        ...
        bindCanvas:function(){
            ...
            function zoomInOut(setSize,zoomFlag){
                ...
                if(zoomFlag===true){
                    ...
                    if((self._$canvasW- self._imgCropSize)/2-self._img_sx<zoomsize){ return="" false;="" }="" if((self._$canvash-="" self._imgcropsize)="" 2-self._img_sy<zoomsize){="" size="-setSize;" self._img_sx="self._img_sx+setSize/2;" self._img_sy="self._img_sy+setSize/2;" self._imgw="self._img.width+size;" self._imgh="self._imgW/self._imgScale;" self._img.width="self._imgW;" self._img.height="self._imgH;" 清除上一次绘制的图片区域="" self.clearcanvas();="" 绘制新的图片区域="" self._$canvas.ctx.drawimage(self._img,self._img_sx,self._img_sy,self._imgw,self._imgh);="" 绘制canvas上的遮罩层="" self.drwashade();="" 绘制剪裁的canvas="" self.drawcanvascrop();="" 绘制预览的canvas="" self.drawcanvaspreview();="" jquery="" 兼容的滚轮事件="" $(document).on("mousewheel="" dommousescroll",''+self._imgbox.selector+'',="" function="" (e)="" {="" var="" delta="(e.originalEvent.wheelDelta" &&="" (e.originalevent.wheeldelta=""> 0 ? 1 : -1)) ||  // chrome & ie
